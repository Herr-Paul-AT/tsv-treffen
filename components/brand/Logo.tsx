 ...
 *
 * Source: handoff/ds/logo.jsx
 */

type MarkVariant = 'color' | 'mono' | 'negative';
...
const PALETTES: Record<MarkVariant, Palette> = {
  color: {
    stone: '#3E4448',
    lake: '#3A6985',
    sand: '#C39265',
...
export type TSVMarkProps = {
  size?: number;
  variant?: MarkVariant;
  className?: string;
};

export function TSVMark({ size = 96, variant = 'color', className = '' }: TSVMarkProps) {
  ...
  const clipId = `tsvmark-clip-${size}-${variant}`;
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="TSV Schloss Treffen"
      className={className}
    >
      <circle cx="80" cy="80" r="74" fill="none" stroke={c.frame} strokeWidth="2.5" />
      <circle cx="80" cy="80" r="66" fill="none" stroke={c.frame} strokeWidth="0.6" opacity="0.4" />
      <defs>
        <clipPath id={clipId}>
      ...
      <g clipPath={`url(#${clipId})`}>
        ...
        <path
          d="M90.5 50 Q104 62 117.5 50"
          fill="none"
          stroke={seamCol}
          strokeWidth="1.6"
          strokeLinecap="round"
          opacity={isMono || isNeg ? 0.9 : 0.8}
        />
        <path
          d="M90.5 50 Q104 38 117.5 50"
          fill="none"
          stroke={seamCol}
          strokeWidth="1.0"
          strokeLinecap="round"
          opacity={isMono || isNeg ? 0.35 : 0.3}
        />
        <path
          d="M14 100 L40 70 L62 88 L88 50 L120 80 L146 95 L146 110 L14 110 Z"
          fill={c.forest}
          opacity={isMono || isNeg ? 1 : 0.92}
        />
        <path
          d="M14 102 L34 88 L56 96 L78 84 L102 96 L124 90 L146 102 L146 110 L14 110 Z"
          fill={c.forest}
          opacity={isMono || isNeg ? 0.6 : 0.55}
        />
        <g fill={c.stone}>
        ...
        <path d="M22 130 L138 130" stroke={c.lake} strokeWidth="2.2" strokeLinecap="round" opacity="0.7" />
        <path d="M30 138 L130 138" stroke={c.lake} strokeWidth="1.8" strokeLinecap="round" opacity="0.45" />
      </g>
    </svg>
  );
}
...
export type TSVLockupProps = {
  height?: number;
  color?: string;
  accent?: string;
};

export function TSVLockup({ height = 36, color = '#3E4448', accent = '#C39265' }: TSVLockupProps) {
  return (
